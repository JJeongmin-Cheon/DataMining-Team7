...
본 프로젝트는 "공휴일이나 축제가 있으면 생활인구가 증가한다"는 일반적인 가설을 **데이터 기반으로 수치화하고**,  
생활인구의 증감 패턴을 다양한 공공 데이터를 활용해 **정량적으로 예측**하는 데 목적이 있습니다.

우리는 단순 상관관계 이상의 설명력을 확보하고자 기상, 공휴일, 문화행사 등 복합 요인을 반영한 회귀 모델을 구성하였으며,  이를 통해 특정 시점의 생활인구 변동을 **상대적 정규화된 수치로 예측**함으로써,  
...
* **축제 및 공휴일 변수**는 생활인구 예측에 일정 수준 기여하며, 특히 무료 문화행사 시 생활인구 증가 경향이 나타남
* **기상 변수**(강수량, 일조시간 등)는 단일 변수로는 큰 영향이 없지만, 다른 변수와의 상호작용을 통해 일부 효과 확인
* **지하철 승하차량 등 실시간 파악 어려운 변수**는 학습에서 제외함으로써 일반화 성능을 확보
* **행정동별 정규화 방식**이 예측 안정성과 해석력 확보에 가장 효과적이었으며, 지역별 상대적 생활인구 변화를 반영 가능

---

## 🏛️ 기대 효과 및 활용 방안

* 특정 날짜 및 지역의 **생활인구 변동을 사전 예측**하여, 공공 자원 배치나 행사 일정 조정에 참고 자료로 활용 가능
* 문화행사, 공휴일 등에 따른 **인구 증감 패턴 분석**을 통해 정책 수립 및 행정 기획에 기여
* 유료/무료 행사 구분 및 빈도 등 정량적 변수 기반 예측을 통해 **이벤트 효과 분석** 가능
* 상권 분석을 위한 정밀 지표는 아니지만, **잠재 유입 인구 규모 파악** 등 간접적인 참고 지표로 활용 가능
...
데이터 병합 과정에서는 중간중간 잦은 데이터셋 rename 및 데이터 삭제가 있었습니다.
